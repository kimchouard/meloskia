...
class ParticleSystemInstance<T extends d.AnyWgslData> {
  ...
  constructor(root: TgpuRoot, outputFormat: GPUTextureFormat, options: ParticleSystemOptions<T>) {
    this.#root = root;
    this.#getInitialPosition = options.getInitialPosition ?? DEFAULT_INIT_POSITION;
    this.#getInitialVelocity = options.getInitialVelocity ?? DEFAULT_INIT_VELOCITY;
    this.#getInitialAge = options.getInitialAge ?? DEFAULT_INIT_AGE;
    this.options = options;

    this.particlesBuffer = root.createBuffer(
    ...
    this.#computePipeline = options.computePipeline(root, {
      $$: {
        get deltaTime() {
...
class ParticleSystem<T extends d.AnyWgslData> {
  ...
  create(root: TgpuRoot, outputFormat: GPUTextureFormat): ParticleSystemInstance<T> {
