...
const BackingAudioManager = ({
  ...
  const getRateForSounds = (currentBPM: number, originalBPM: number): number =>
  ...
  const updateGainNodeVolume = (
    ...
    verbose &&
      console.log(
        `[BackingAudioManager/updateGainNodeVolume] Updated sound ${soundName} gain node volume:`,
        {
          baseVolume,
          trackVolume,
          finalVolume,
  ...
  const loadSound = async (soundName: SoundName, isLooping = false) => {
    ...
    if (!soundObj) {
      console.error(
        `[BackingAudioManager/loadSound] No sound found with this name: "${soundName}".`,
    ...
    if (soundObj.status === 'loaded' || soundObj.status === 'disabled') {
      ...
      verbose &&
        console.log(
          `[BackingAudioManager/loadSound] Sound "${soundName}" is already loaded or disabled.`,
          { soundObj }
        );
    ...
    if (!audioBuffer) {
      console.error(
        `[BackingAudioManager/loadSound] Can't load sound "${soundName}": url not defined`,
        { sourceUrl: soundData.url, audioContext, audioBuffer }
      );
    ...
    if (!gainNode) {
      console.error(
        `[BackingAudioManager/loadSound] Can't create gain node for sound "${soundName}"`
      );
      throw new Error(
        `Error while creating gain node for sound "${soundName}".`
      );
    }
    ...
    if (!playerNode) {
      console.error(
        `[BackingAudioManager] Can't create player node for sound "${soundName}"`
      );
      throw new Error(
        `Error while creating player node for sound "${soundName}".`
      );
    }
  ...
  const loadSounds = async () => {
    ...
    for (const soundName of Object.keys(soundObjs.current) as SoundName[]) {
      ...
      // if (soundData && soundData.url) {
      verbose &&
        console.log(`[BackingAudioManager] Loading sound: ${soundName}`);
  ...
  const playSounds = async (options?: {
    trackList?: SoundName[];
    startAtTime?: number;
  }) => {
      ...
      if (soundObjs.current[soundName]?.status === 'loaded') {
        ...
        if (playerNode) {
          try {
            ...
            verbose &&
              console.log(
                `[BackingAudioManager/playSounds] Started sound ${soundName} at time ${startTime}`,
                { playerNode }
              );
          } catch (error) {
            // reportErrorToSentry(error, 'AudioManager/playSounds');
            console.error(
              `[BackingAudioManager/playSounds] Error playing sound ${soundName}:`,
              error
            );
          }
        }
  ...
  const stopSound = async (soundName: SoundName) => {
    ...
    try {
      ...
      if (!playerNode || isSoundDisabled(soundName)) {
        console.error(
          `[AudioManager/stopSound] Attempted to stop non-existent or disabled sound: ${soundName}`
        );
        return;
      }
      ...
      verbose &&
        console.log(
          `[BackingAudioManager/stopSound] Successfully stopped sound ${soundName} at audioContext.currentTime: ${audioContext.currentTime}`
        );
    } catch (e) {
      //// reportErrorToSentry(e, 'AudioManager/stopSound');
      console.error(
        `[BackingAudioManager/stopSound] Error while stopping sound ${soundName}:`,
        e
      );
    }
    ...
    if (!newPlayerNode) {
      console.error(
        `[AudioManager] Can't recreate player node for sound "${soundName}"`
      );
      throw new Error(
        `Error while recreating player node for sound "${soundName}".`
      );
    }
  ...
  const updatePlayerNodeRate = async (
    audioContext: AudioContext,
    soundName: SoundName
  ) => {
    const soundObj = soundObjs.current[soundName];
